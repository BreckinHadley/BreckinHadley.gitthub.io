---
title: "Sentiment Analysis with Star Wars"
---
...
For this plot I want to know the top 10 most frequent positive words and top 10 most frequent negative words used in Star Wars Episodes 4-6.
...
**Insights:**
This plot produces similar results as the plot above but looks at the top 10 most frequent trusting words and top 10 most frequent fearful words used in Star Wars Episodes 4-6. 
...
SW_tidy |>
  ...
  slice_max(order_by = total_count, n = 10, with_ties = FALSE) |>
  gt() |>
  tab_header(
    title = "Top 10 Most Frequent Non-Stop Words",
    subtitle = "Star Wars Dialogue Analysis"
  ) |>
  cols_label(
    word = "Word",
    total_count = "Count"
  ) |>
  fmt_number(columns = total_count, decimals = 0) |>
  tab_options(
    table.font.size = 14,
...
**Insights:**
This plot was one of my favorites to make. This is a sentiment trajectory plot for each Star Wars movie which basically shows the emotional trajectory for each of the films. All of the films seem to be relatively similar to one another with one not being dratically differnt from the others.
